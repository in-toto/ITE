...
ITE-4 compliant in-toto tools will require a strategy for determining what resources
these generic URIs are pointing to. Such resolvers, as we refer to these strategies, are required so
...
`hash_artifacts` closely mirror the general workflow of hashing artifacts in
the filesystem, and are similar to the way in-toto reference implementations
...
implementer, or perhaps GitHub, chooses to define the components of a pull request.
